...
workflow NALLO {
    ...
    family_snv_vcf      = params.skip_rank_variants ? channel.empty() :  CONCAT_SORT_RANKED_SNVS.out.vcf
    family_sv_vcf_tbi   = params.skip_rank_variants ? channel.empty() :  RANK_VARIANTS_SVS.out.vcf.join(RANK_VARIANTS_SVS.out.tbi)
    family_str_vcf_tbi  = params.skip_repeat_annotation ? channel.empty() : !params.skip_repeat_calling ? ANNOTATE_REPEAT_EXPANSIONS.out.vcf.join(ANNOTATE_REPEAT_EXPANSIONS.out.tbi) : channel.empty()
